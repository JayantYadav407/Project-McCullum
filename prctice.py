...
def extract_and_save_to_csv():
    ...
    # print(soup).text
    # Extract temperature using 'style="display:inline"' attribute
    temperature_element = soup.find_all("div", class_="UQt4rd")
    print(temperature_element)
    temperature = temperature_element.get_text(strip=True) if temperature_element else "N/A"

    if temperature == "N/A":
        print("Temperature data not found. Inspect the saved HTML file for correct selectors.")
        return
